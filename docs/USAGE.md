...
|----------|-------|---------|-------------|
| `generators-*` | 63 | Generate code | Add features to any app |
| `product-*` | 13 | Idea to specs | New app planning, beta testing, localization |
| `swiftui-*` | 11 | SwiftUI features | Layout, environment, focus, observable, presentations, transferable, AlarmKit, WebKit, text editing, toolbars, Charts 3D |
| `ios-*` | 9 | iOS development | Code review, UI review, architecture, accessibility, navigation, iPad, migration |
| `macos-*` | 8 | macOS development | Code review, UI review, AppKit-SwiftUI bridge, Tahoe APIs |
| `swift-*` | 8 | Swift language | Concurrency, Codable, macros, memory, testing, Package Manager, regex, CLI apps |
| `testing-*` | 8 | TDD & testing | TDD workflows, snapshots, test infrastructure |
| `app-store-*` | 7 | ASO & marketing | Descriptions, keywords, search ads, rejections, marketing |
| `growth-*` | 4 | User acquisition | Analytics, press, community, indie business |
...
| `swiftdata-*` | 2 | Data persistence | Patterns, class inheritance |
| `visionos-*` | 2 | visionOS | Spatial computing, widgets |
| `legal-*` | 1 | Legal documents | Privacy policies |
| + 21 standalone | 21 | Frameworks & tools | Core ML, ARKit, AVFoundation, CarPlay, GameKit, HealthKit, MapKit, Monetization, MusicKit, PassKit, WeatherKit, watchOS, tvOS, etc. |

**Total: 166 skills across 17 categories + 21 standalone**
