---
name: generators-test-generator
...
- Asks to "add tests" or "write tests" for existing code
- Asks about unit testing, UI testing, or XCTest setup
- Wants to test ViewModels, services, or repositories
- Wants mock objects or test helpers generated

**Do NOT activate** for these — use the specialized skill instead:
- TDD workflow (test-first development) → `testing-tdd-feature` or `testing-tdd-bug-fix`
- Swift Testing API details (`@Test`, `#expect`, traits, parameterized tests) → `swift-testing`
- Snapshot/visual regression tests → `testing-snapshot-test-setup`
- Pre-refactor safety net → `testing-tdd-refactor-guard` or `testing-characterization-test-generator`
- Protocol contract tests → `testing-test-contract`
- Test data factories → `testing-test-data-factory`
- Integration test harness with mock servers → `testing-integration-test-scaffold`
