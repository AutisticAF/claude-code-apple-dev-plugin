...
- `user-invocable`: Set to `true` if the skill can be triggered via slash command
- `argument-hint`: Hint text shown when the user types the slash command
- `compatibility`: Compatibility constraints (e.g., platform requirements)
- `disable-model-invocation`: Set to `true` to prevent automatic activation by the model
...
- **references/skill-template.md** - Basic skill template
- **references/complex-skill-template.md** - Modularized skill template
